...
So maybe we could instead optimize (over $g$) `calc_KL(x, s, g) - 0.5 * sum((1 / s^2) * (Etheta2(x, s, g) - 2 * x * Etheta(x, s, g))`? If the quantity calculated by `calc_KL` is indeed correct, then this is simply another way to write the objective $F(g, q)$ (with the constraint that $q$ is the posterior induced by $g$).
