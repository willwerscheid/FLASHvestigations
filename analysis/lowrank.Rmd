...
# Converts rsvd output to a lowrank object for input to flashier.
svd.to.lr <- function(svd) {
  lr <- list(t(t(svd$u) * svd$d), svd$v)
  class(lr) <- "lowrank"
  return(lr)
}
...
noise.lr50 <- svd.to.lr(noise.svd50)
noise.fl.lr <- flashier(noise.lr50, greedy.Kmax = 5)
```
...
noise.fl.lr.addvar <- flashier(noise.lr50, S = S)
```
...
four.factor.lr50 <- svd.to.lr(four.factor.svd50)
four.factor.fl.lr <- flashier(four.factor.lr50, greedy.Kmax = 10)
...
four.factor.fl.lr.addvar <- flashier(four.factor.lr50, S = S)
...
overlap.lr <- svd.to.lr(overlap.svd50)
S <- sqrt(calc.unexplained.var(overlap, overlap.svd50))
overlap.lr.fl <- flashier(overlap.lr, S = S)
