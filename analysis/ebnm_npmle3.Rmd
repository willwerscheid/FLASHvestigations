...
t_oneshot <- system.time({
  ...
  cat("Component length:", round(comp_len, 3), "Likelihood:", ebnm_res2$log_likelihood, "\n")
})
...
ggplot(cdf_df, aes(x = x, y = y, col = method)) + 
  geom_line()
```
The log likelihood of the one-shot method is again a bit worse. I check both results by removing the zero components and re-estimating (this should reduce the rounding error).
...
g_init <- ashr::unimix(pi = ebnm_res2$fitted_g$pi[nz_comp],
...
So it's not simply numerical error. I'm not sure what's going on here. The grids are very slightly different, but not by much:

```{r check2}
cat("Amount of non-overlap for each component obtained using the refinement method:\n")
sapply(ebnm_res$fitted_g$a, function(x) min(abs(x - ebnm_res2$fitted_g$a))) +
  sapply(ebnm_res$fitted_g$b, function(x) min(abs(x - ebnm_res2$fitted_g$b)))
```
