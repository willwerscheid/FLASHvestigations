...
So it's not simply numerical error. I'm not sure what's going on here, but `mixsqp` is not finding the best solution. I provide the verbose output of the two checks for inspection.

```{r mixsqp}
g_init <- ashr::unimix(pi = rep(1 / ncomp, ncomp),
                       a = min(samp) + 0:(ncomp - 1) * comp_len,
                       b = min(samp) + 1:ncomp * comp_len)
ebnm_res2 <- ebnm::ebnm_npmle(samp, g_init = g_init, control = list(verbose = TRUE))
```
