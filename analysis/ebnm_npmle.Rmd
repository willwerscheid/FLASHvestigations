...
I want to see how grid density affects convergence properties and the quality of the solution. From a log likelihood perspective, using a grid of points spaced at a distance that's about half the standard deviation of the noise gives a solution that is pretty much just as good as a very fine grid. Note that the Dicker and Zhao recommendation -- $\sqrt{n} \approx 32$, which corresponds to a `scale` of a little more than 0.8 -- produces a noticeably worse solution. 
