...
I suggest the following procedure. [Recall](https://willwerscheid.github.io/MASHvFLASH/MASHvFLASHgtex2.html) that we are primarily interested in obtaining a good loadings matrix and accurate priors on the factors. Then, with loadings and priors on factors fixed, estimating posteriors for individual tests is entirely straightforward.
...
1. For each successive mini-batch $Y_j$:
    ...
    1. Take the loadings from the previous FLASH object $f^{(j - 1)}$. Fix them and fit a new FLASH object $\tilde{f}^{(j)}$ to the new mini-batch $Y_j$, initializing the priors on the factors to the values obtained for the previous FLASH object $f^{(j - 1)}$ (in particular, this ensures that the ASH grid does not change from one object to another).
...
1. If desired, one can iterate through the complete dataset (or part of it) a second time (optionally, re-permuting the dataset). If this is done, then the weights in the above averages can be fixed at $(m - 1) / m$ and $1 / m$ (recall that $m$ is the number of mini-batches).
